...
SubscriberNameFeedbackDrive = 'feedback/drive'
# Param
pos_type = 'circle'  # 'line', 'point', 'curve', 'circle', 'orign', 'initial_orign'
# delta_t = 0.001  # 时间间隔
# iteration = 8000  # 迭代
# density = 100  # 图像点的密度
# save2csv = False
...
J_inv = matrix([[1., -(l / 2.)], [1., (l / 2.)]]) * (1. / r)
J_for = matrix([[(1. / 2.), (1. / 2.)], [-(1. / l), (1. / l)]]) * (r)
...
class Trajectory_Tracking_SMC(ControllerPositionClass, ControllerHeadingClass,
    ...
    def odom(self):
        ...
        dot_theta_amp_l = self.Motor_UCR.trajectory_motor(node_id=0)
        dot_theta_amp_r = self.Motor_UCR.trajectory_motor(node_id=1)

        dot_theta = matrix([dot_theta_amp_l, dot_theta_amp_r])
