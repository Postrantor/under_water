...
PublisherNameControlDrive = 'control/impedance_drive'
PublisherNameFeedbackDrive = 'feedback/impedance_drive'
...
class MethodsClass(object):
    ...
    K = 10.  # 蜗轮蜗杆的减速比`10:1`
    Counts = K * Maxon_306090.Counts_per_Reduction  # 3,522,560
    # Counts = K * Maxon_305474.Counts_per_Reduction # 920,000
...
class ControlsToMotors(DebugSteam, BagPathClass, MethodsClass):
    ...
    def inits_Parameter(self):
        # Sample
        self.rate = rospy.get_param('/control/diff_drive/rate', 50)
        # 5Hz - 163 - 3
        # 7Hz - 211 - 2
        # 10Hz - 311 - 1
    # Msg_Subscriber
...
# ==================================================
    ...
    # 1. 获取反馈值
        ...
        # print("直接输出位置(count)：{}".format(self.amp_pos))
    ...
    # 2.调用算法(若未经过算法调整，则直接返回订阅指令)
        ...
        amp_pos_rad = self.count2rad(self.amp_pos)
        amp_vel_rad = self.count2rad(self.amp_vel)
        # self.debug_stream(amp_pos_rad)
        # 3. 调用阻抗控制算法
        tar_pos, tar_vel, tar_acc = self.Alg_Impedance.update(amp_pos_rad, amp_vel_rad)
        # 3. 单位转换
        self.amp_pos = self.rad2count(tar_pos)
        self.amp_vel = self.rad2count(tar_vel)
        # self.debug_stream(self.amp_pos)
        self.tar_val = self.amp_pos
        # 4. 对阻抗控制计算值进行KF滤波，之后再发送给电机执行指令，看看会不会平滑一些
        # tar_vel_kf = self.kf_velocity_tar.step(0, tar_vel)
        # print("直接输出位置(rad)：{}".format(amp_pos_rad))
        # print("阻抗控制位置(rad)：{}".format(tar_pos))
        # print("阻抗控制位置(count)：{}".format(self.amp_pos))
    # 3. 控制量
        ...
        # self.Motor_Wing.control_motor_pos_vel(position=self.amp_pos, velocity=self.amp_vel, acceleration=1000000, node_id=0)
        self.Motor_Wing.control_motor_pos(0, 0, 0, target=self.amp_pos, node_id=0)
# ==================================================
#                                         Publisher_Msg
# ==================================================
    ...
    # [issue]: 这行是否有必要
        ...
        self.control_msg.position.wing.motor_l = self.tar_val
    # Publish
    ...
    # Feedback Position
        if stop:
            ...
            self.feedback_msg.current.wing.motor_l = self.amp_cur
            self.feedback_msg.velocity.wing.motor_l = self.amp_vel
            self.feedback_msg.position.wing.motor_l = self.amp_pos
    # Publish
