...
'''md
    [20210904]:
        目前使用的RS232通讯方式不能实现同时控制两台电机的阻抗控制，采样频率太低，无法收敛
        先通过控制单侧进行实验，后面改成CANOpen之后再做全方位升级
        包括ROS2、实时控制插件也要提上日程
    [20210902]:
        1. 修改端口等相关配置，将该程序移植到机器人上，之前是在但电机上测试
        2. 增加bag功能，采集传感器数据(编码器、电流)，方便放在论文里面分析
        [注]: 另外需要对`Controller_SwitchDrive.py`程序进行分离，原来是控制四个电机，需要修改成控制两个电机
            另外两个电机分给阻抗控制程序。
            整体的运行思路是，将`Controller_SwitchDrive.py`分离出来，但是保持原来的功能，因为还需要通过手柄将机构回位到负极限
            之后，将控制推拉机构的两个电机的程序关闭；
            使用阻抗控制程序接管，控制机构运动
        3. 修改为同时控制左右两个电机运动
    [20210819]: 
        关于阻抗控制
...
PublisherNameControlDrive = 'control/impedance_wing'
PublisherNameFeedbackDrive = 'feedback/impedance_wing'

# %% class
class MethodsClass(object):
    '''
        [20210901]:
            移植到机器人上的话，增加了蜗轮蜗杆的减速比，所以这里的系数也应该相应的变化
        [README]:
    ...
    K = 10 # 蜗轮蜗杆的减速比`10:1`
    Counts = K * Maxon_306090.Counts_per_Reduction # 352256
    # Counts = K * Maxon_305474.Counts_per_Reduction
...
# ==================================================
    ...
    # Sample
        self.rate = rospy.get_param('/control/wing/rate', 10)
        # 5Hz - 163 - 3
        # 7Hz - 211 - 2
        # 10Hz - 311 - 1
    # Msg_Subscriber
    ...
    def inits_Motor(self):
        ...
        self.Motor_Wing = Motor.MotorClass(PortID=PortID_UCR, # PortID_Wing, 
                                                                        ...
                                                                        profile=0, 
                                                                        pos=0, 
                                                                        vel=Maxon_306090.Max_Vel, 
                                                                        acc=100000, dec=100000)
    def inits_AlgImpedance(self):
        ...
        '''
        self.Alg_Impedance_l = AlgorithmImpedanceClass()
        self.Alg_Impedance_l.Button = True
        ...
        self.Alg_Impedance_r = AlgorithmImpedanceClass()
        self.Alg_Impedance_r.Button = True
    def inits_AlgKalmanFilter(self):
        ...
        '''
        self.kf_current_l = SingleStateKalmanFilter(x=0, P=10.0, A=1.0, B=0.0, C=1.0, Q=0.005, R=1.0)
        self.kf_velocity_l = SingleStateKalmanFilter(x=0, P=10.0, A=1.0, B=0.0, C=1.0, Q=1.0, R=50.0)
        self.kf_velocity_tar_l = SingleStateKalmanFilter(x=0, P=10.0, A=1.0, B=0.0, C=1.0, Q=1.0, R=50.0)
        self.kf_current_r = SingleStateKalmanFilter(x=0, P=10.0, A=1.0, B=0.0, C=1.0, Q=0.005, R=1.0)
        self.kf_velocity_r = SingleStateKalmanFilter(x=0, P=10.0, A=1.0, B=0.0, C=1.0, Q=1.0, R=50.0)
        self.kf_velocity_tar_r = SingleStateKalmanFilter(x=0, P=10.0, A=1.0, B=0.0, C=1.0, Q=1.0, R=50.0)
# ==================================================
#                                           Callback_Msg
# ==================================================
...
# ==================================================
    ...
    # 1. 获取反馈值
        ...
        feedback_l = self.Motor_Wing.feedback_motor(node_id=0)
        self.amp_cur_l = feedback_l['current_amp']
        self.amp_vel_l = feedback_l['velocity_amp']
        self.amp_pos_l = feedback_l['position_amp']
        # get feedback from other senser
        # 
    # 2.调用算法(若未经过算法调整，则直接返回订阅指令)
        # 1. 通过`Kalman Filter`对电流值进行滤波
        # 虽然当前并没有通过电流值作为反馈，但是还是要进行滤波处理的，不然噪声太大，根本没法看
        self.amp_cur_l = self.kf_current_l.step(0, self.amp_cur_l)
        # tau_e = 30000*amp_cur_kf
        # self.amp_vel = self.kf_velocity.step(0, self.amp_vel)
        # 2. 单位转换
        # [issue]: 单位转换函数后面考虑转移到微分运动学类里面，直接返回对应的数值
        feedback_pos_rad_l = self.count2rad(self.amp_pos_l)
        feedback_vel_rad_l = self.count2rad(self.amp_vel_l)
        # self.debug_stream(amp_pos_rad)
        # 3. 调用阻抗控制算法
        tar_pos_l, tar_vel_l, tar_acc_l = self.Alg_Impedance_l.update(feedback_pos_rad_l, feedback_vel_rad_l)
        # 3. 单位转换
        # self.amp_pos = self.rad2count(tar_pos)
        # self.amp_vel = self.rad2count(tar_vel)
        # self.debug_stream(self.amp_pos)
        self.tar_pos_l = self.rad2count(tar_pos_l)
        self.tar_vel_l = self.rad2count(tar_vel_l)
    ...
    # 3. 控制量
        ...
        # 但是位置模式目前存在问题，暂不使用
        # self.Motor_Wing.control_motor_vel(target=self.tar_vel_l, node_id=0)
        self.Motor_Wing.control_motor_pos(0,0,0,target=self.tar_pos_l,node_id=0)
    def update_callback_r(self):
        """
            [issue]:
            在算法应用层中，尽可能的用传参以及return的方式。少用self
            这样到底有哪些是输入，那些是输出就清楚得多
    ...
    # 1. 获取反馈值
        # get feedback from motor
        feedback_r = self.Motor_Wing.feedback_motor(node_id=1)
        self.amp_cur_r = feedback_r['current_amp']
        self.amp_vel_r = feedback_r['velocity_amp']
        self.amp_pos_r = feedback_r['position_amp']
        # get feedback from other senser
        # 
    # 2.调用算法(若未经过算法调整，则直接返回订阅指令)
        # 1. 通过`Kalman Filter`对电流值进行滤波
        # 虽然当前并没有通过电流值作为反馈，但是还是要进行滤波处理的，不然噪声太大，根本没法看
        self.amp_cur_r = self.kf_current_r.step(0, self.amp_cur_r)
        # tau_e = 30000*amp_cur_kf
        # self.amp_vel = self.kf_velocity.step(0, self.amp_vel)
        # 2. 单位转换
        # [issue]: 单位转换函数后面考虑转移到微分运动学类里面，直接返回对应的数值
        feedback_pos_rad_r = self.count2rad(self.amp_pos_r)
        feedback_vel_rad_r = self.count2rad(self.amp_vel_r)
        # self.debug_stream(amp_pos_rad)
        # 3. 调用阻抗控制算法
        tar_pos_r, tar_vel_r, tar_acc_r = self.Alg_Impedance_r.update(feedback_pos_rad_r, feedback_vel_rad_r)
        # 3. 单位转换
        # self.amp_pos = self.rad2count(tar_pos)
        # self.amp_vel = self.rad2count(tar_vel)
        # self.debug_stream(self.amp_pos)
        self.tar_pos_r = self.rad2count(tar_pos_r)
        self.tar_vel_r = self.rad2count(tar_vel_r)
        # 4. 对阻抗控制计算值进行KF滤波，之后再发送给电机执行指令，看看会不会平滑一些
        # tar_vel_kf = self.kf_velocity_tar.step(0, tar_vel)
    # 3. 控制量
        # [issue]: 对于速度模式来说，只需要不断发送新的速度值即可
        # 不过还是应该改成位置模式！需要修改之前的位置模式下的参数，增加最大速度限制，同时将速度指令和位置指令发送给电机
        # 但是位置模式目前存在问题，暂不使用
        # self.Motor_Wing.control_motor_vel(target=self.tar_vel_r, node_id=1)
        self.Motor_Wing.control_motor_pos(0,0,0,target=self.tar_pos_r,node_id=1)
    def update_callback(self):
        self.update_callback_l()
        self.update_callback_r()
# ==================================================
#                                         Publisher_Msg
# ==================================================
    ...
    # Msg
        # [issue]: 这行是否有必要
        if stop:
            self.tar_vel_l = 0 # stop motor's speed
            self.tar_pos_l = 0
            self.tar_vel_r = 0 # stop motor's speed
            self.tar_pos_r = 0
        self.control_msg.position.wing.motor_l = self.tar_pos_l # 记录阻抗控制的位置指令
        self.control_msg.velocity.wing.motor_l = self.tar_vel_l # 记录阻抗控制的速度指令
        self.control_msg.position.wing.motor_r = self.tar_pos_r # 记录阻抗控制的位置指令
        self.control_msg.velocity.wing.motor_r = self.tar_vel_r # 记录阻抗控制的速度指令
    # Publish
    ...
    # Msg
        if stop:
            ...
            self.feedback_msg.current.wing.motor_l = self.amp_cur_l
            self.feedback_msg.velocity.wing.motor_l = self.amp_vel_l
            self.feedback_msg.position.wing.motor_l = self.amp_pos_l
            self.feedback_msg.current.wing.motor_r = self.amp_cur_r
            self.feedback_msg.velocity.wing.motor_r = self.amp_vel_r
            self.feedback_msg.position.wing.motor_r = self.amp_pos_r
    # Publish
