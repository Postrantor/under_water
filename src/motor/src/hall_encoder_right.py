...
class HallEncoderCounter:
    ...
    # ==================================================
    def __init__(self):
        rospy.init_node('hall_encoder_right')
        # 初始化参数
        self.Counter = 0
        self.flag = 0
        self.Hall_B_Last = 0
        ...
        self.rate = 10
        # 声明一个Publisher
        self.rwheel_enc_pub = rospy.Publisher('rwheel_enc', Int64, queue_size=10)

        # 指定GPIO引脚规则：BOARD、BCM
        GPIO.setmode(GPIO.BCM)
        # 定义引脚常量
        self.Hall_A = 16  # Right_A相
        self.Hall_B = 17  # Right_B相
        # 初始化引脚状态
    ...
    def update(self):
        ...
        while not rospy.is_shutdown():
            ...
            if tmp != Counter:
                ...
                tmp = self.Counter
                self.rwheel_enc_pub.publish(Counter)
            # rate.sleep()
        # rospy.spin()
