...
class BME280():
    ...
    def __init__(self, bus=1):
        ...
        self.bus = smbus.SMBus(bus)  # Rev 2 Pi, Pi 2 & Pi 3 uses bus 1 # Rev 1 Pi uses bus 0
    ...
    def readBME280ID(self, addr=DEVICE):
        ...
        return {'chip_id': chip_id, 'chip_version': chip_version}
    ...
    def readBME280All(self, addr=DEVICE):
        ...
        return {'temp': temperature / 100.0, 'psr': pressure / 100.0, 'hum': humidity}
...
def main():
    ...
    print('Chip ID     :', read_id['chip_id'])
    print('Version     :', read_id['chip_version'])

    read_all = bme280.readBME280All()
    print('Temperature : ', read_all['temp'], '°C')
    print('Pressure : %.5f' % read_all['psr'], 'hPa')
    print('Humidity : %.5f' % read_all['hum'], '%')
